...
class AppPage(tk.Frame):
	...
	def clicked_delete_btn(self):
		...
		if confirm:
			...
			del self.settings.database1[next(islice(self.settings.database1, index, None))]
			self.settings.save_data1_to_json()
			self.last_current_data_index = 0
	...
	def clicked_save_data_btn(self):
		...
		if confirm:
			...
			#productid = self.entry_update_data_vars[2]
			...
			self.settings.database[index] = {
				productid : {
					...
					"stok" : stok,
					"supplier" : supplier,
					"lastchanged" : datetime.now().strftime("%Y-%m-%d %H:%M:%S")
				}
			}
			self.settings.save_data_to_json()
		self.current_data = self.settings.database[index]
	...
	def clicked_save_add_new_data_btn(self):
		...
		if confirm:
			...
			stok = self.entry_update_data_vars[2].get()
			supplier = self.entry_update_data_vars[3].get()
			testsame = self.settings.checksame(productid)
			while testsame:
				productid += 1
				testsame = self.settings.checksame(productid)
			else:
				...
				new_data1 = {
					productid : {
						"dummydata" : dummydummy
					}
				}
				self.settings.database1.update(new_data1)
				self.settings.save_data1_to_json()
